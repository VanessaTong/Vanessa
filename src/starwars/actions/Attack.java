...
import starwars.entities.ForceAbilityRequirement;
...
public class Attack extends SWAffordance implements SWActionInterface {
	 ...
	 * 	<li>The <code>SWActor a</code> is holding an item without the <code>WEAPON</code> capability </li>
	 *  <li>The <code>SWActor a</code> is holding an item with the <code>WEAPON</code> capability, but
	 *  	the item requires a Force ability level to use, and the <code>SWActor a</code> \
	 *  	does not have this level</li>
	 * </ul>
	 * <p>
	...
	public void act(SWActor a) {
		...
		if (targetIsActor && (a.getTeam() == targetActor.getTeam())) { //don't attack SWActors in the same team
			...
			if (itemCarried != null) {//if the actor is carrying an item
				...
				// can we wield the item?
				// i.e. is it a weapon, and if so, do we have the ability (if required)?
				boolean canWield = itemCarried.hasCapability(Capability.WEAPON);
				if (canWield && itemCarried instanceof ForceAbilityRequirement) {
						int forceRequirement = ((ForceAbilityRequirement) itemCarried).getForceAbilityRequirement();
						int forceAbility = a.getForceAbility();
						canWield = forceAbility >= forceRequirement;
				}				
				if (canWield) {					
